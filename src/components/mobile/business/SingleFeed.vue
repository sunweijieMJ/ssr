<template>
  <div v-if="vitem" class="single-feed" @click="skipDetail(vitem.entity_id, vitem.entity_type)">
    <!-- 列表头部用户信息 -->
    <div class="list-header">
      <div class="header-author">
        ...
        <div class="author-name" :class="{'null-name': !(vitem.name || vitem.entity_user_info.signiture)}">
          ...
          <p v-if="vitem.name || vitem.entity_user_info.signiture" :class="{focus: ($route.name === 'Choiceness' || $route.name === 'Moment' || $route.name === 'TopicDetail')}">
            <span v-if="vitem.name">{{vitem.name}}</span>
            <span v-else>{{vitem.entity_user_info.signiture}}</span>
          </p>
        </div>
      </div>
      <focus-btn v-if="$route.name === 'Choiceness' || $route.name === 'Moment' || $route.name === 'TopicDetail'"></focus-btn>
    </div>
    <!-- 文本内容 -->
    <div class="list-main">
      ...
      <div class="main-images" v-if="vitem.entity_photos && vitem.entity_photos.length">
        ...
        <div class="mark" v-if="(vitem.with_video == 1 && $route.name === 'MomentDetail') || vitem.entity_type != 6">
    ...
    <!-- 兑换条 -->
    <ul class="bound-bar" v-if="vitem.entity_extra && vitem.entity_extra.hangings && vitem.entity_extra.hangings.total && $route.name !== 'ActivityDetail' && $route.name !== 'ActivityShow' && $route.name !== 'ProductDetail' && $route.name !== 'BuyerShow'">
      <li v-for="(witem, windex) in $route.name === 'MomentDetail' ? vitem.entity_extra.hangings.items : vitem.entity_extra.hangings.items.slice(0, 1)" :key="windex" v-if="witem.show_status" @click.stop="skipDetail(witem.object_id, witem.type)">
        <div class="msg">
...
<script>
  ...
  export default {
    mixins: [frequent],
    props: ['vitem', 'stick'],
    components: {FocusBtn, Paragraph, VueVideo, Images},
    data() {
      return {
        imageSize,
        timeFilter,
        parHeight: true, // ETC 段落高度
        showText: false, // ETC 全文按钮是否显示
        muted: true // ETC 静音
      };
    },
    beforeMount() {
    ...
    methods: {
      ...
      limitHeight() {
        let that = this;
        if(that.$route.name === 'MomentDetail') return;
        that.$nextTick(() => {
          if(that.$el.querySelector('.list-footer')) {
            const lineH = that.$el.querySelector('.list-header .author-name h4>span').offsetHeight;
            const p = that.$el.querySelector('.list-main .main-paragraph p');
            if(!p) {
              that.showText = false;
              return;
            }
